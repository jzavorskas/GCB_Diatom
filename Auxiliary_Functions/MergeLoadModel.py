...
Function: Merged (Diatom/Cyanobacteria) Model Loading (MergeLoadModel) function
  ...
  Outputs: 
        ...
        ML : COBRA model for symbiotic Thalassiosira pseudonana and Anabaena variabils,
                with default objective and constraints

  This function loads the symbiotic diatom/cyanobacteria FBA model, and adds in reactions 
  specific to our application. As in the diatom-only model, reactions are inserted that allow 
  the diatom to store fixed carbon, and then retrieve it from storage and convert it to a usable 
  form. New metabolites are also created in tandem with these new reactions. Additionally, transfer
  reactions to move carbon and nitrogen between diatom and cyanobacteria. The default bounds are 
  set, and the objective function is updated to be a sum of the two organisms' growth rates.
"""
...
def MergeLoadModel(Inputs): # model: cyanobacterium Synechocystis sp. PCC 6803; model2: diatom Phaeodactylum tricornutum CCAP 1055/1
    ...
    datamin_re = np.loadtxt(".\\Data\\CarbonTransfer.csv",delimiter=',')
